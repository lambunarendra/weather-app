...
public class WatchListServiceImpl implements IWatchListService{
    ...
    public String generateId(String userName,String name,String country) { // add this to method 
        return userName+"-u-"+country+"-u-"+name; // add this too 
    }
    ...
    public CityWeatherDetails addToWatchList(AddToWatchlist requestData) throws AlreadyExistInWatchListException {
        ...
        Optional<CityWeather> citylist =watchListRepo.findByUserNameAndNameAndCountry(requestData.getUserName(),requestData.getName(),requestData.getCountry()); //add this to 
        if(citylist.isPresent()){
            throw  new AlreadyExistInWatchListException("Already Added in Watch List");
        }
        String generatedId = generateId(requestData.getUserName(),requestData.getName(),requestData.getCountry()); // add this to bracket 
    ...
    public void remove(RemoveFromWatchlist requestData) throws NoUserFoundException {
        Optional<CityWeather> weather = watchListRepo.findByUserNameAndNameAndCountry(requestData.getUserName(),requestData.getName(),requestData.getCountry()); // add this to method 
        if(weather.isEmpty()){
